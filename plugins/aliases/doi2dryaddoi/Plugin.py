...
class PluginClass(BasePluginClass):
    ...
    SOURCE_NAME = "doi2dryaddoi"
    SOURCE_DESCRIPTION = "Looks up alias for entered ID"
    DEBUG = False

    LOOKUP_DRYAD_DOI_URL = "http://datadryad.org/solr/search/select/?q=dc.relation.isreferencedby:%s&fl=dc.identifier"
    
    def get_dryad_doi_from_article_doi(self, article_doi):
        # To get url of full text, try to follow the doi url then get the final landing page
        lookup_url = self.LOOKUP_DRYAD_DOI_URL % article_doi
        #print lookup_url
        
        (header, xml) = self.get_cache_timeout_response(lookup_url)
        soup = BeautifulStoneSoup(xml)
        #print soup
        
        try:
            dryad_doi = soup.result.doc.arr.str.string
    ...
    def build_artifact_response(self, artifact_id):
        doi = self.get_dryad_doi_from_article_doi(artifact_id)
        if doi:
            response = dict(attacheddatadoi=doi)
        else:
            response = {}
        return(response)
