...
TEST_INPUT = '{"http://www.slideshare.net/phylogenomics/eisen":{"doi":"FALSE","url":"http://www.slideshare.net/phylogenomics/eisen","pmid":"FALSE"}}'
...
def get_page(id):
    ...
    query_url = SLIDESHARE_API_URL %(ts, hash_combo, id)
    #print url
    try:
...
def extract_stats(page):
    ...
    downloads = get_as_int(soup.numdownloads)
    views = get_as_int(soup.numviews)
    comments = get_as_int(soup.numcomments)
    favorites = get_as_int(soup.numfavorites)
    try:
...
def is_slideshare_doi(id):
    # Mendeley takes any crossref doi
    response = (SLIDESHARE_URL_PATTERN.search(id) != None)
    return(response)
    
# each plugin needs to write relevant versions of this            
def artifact_type_recognized(id):
    is_recognized = is_slideshare_doi(id)
...
#example = "http://www.slideshare.net/phylogenomics/eisen"
