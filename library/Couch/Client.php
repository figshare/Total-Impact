...
// 		print_r($this->query_parameters);
	...
	public function storeDoc($doc)
	{
		...
		foreach (array_keys(get_object_vars($doc)) as $key) {
			if (in_array($key, Couch_Client::$underscored_properties_to_remove_on_storage)) {
				unset($doc->$key);
			}
			elseif (substr($key, 0, 1) == '_' AND !in_array($key, Couch_Client::$allowed_underscored_properties))
